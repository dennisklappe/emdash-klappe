...
(The `icon`, `screenshots`, and `banner` shapes are deliberately identical to FAIR's WordPress and TYPO3 extensions — with `screenshots` as a list of artifacts; FAIR's singular `screenshot` alias is not included — so directory tooling can render them uniformly across ecosystems.)
